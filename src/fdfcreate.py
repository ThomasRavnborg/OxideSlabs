...
def modify_basis(block_in, symbols, auto_radii=True):
    ...
    def _add_polarization_orb(block_in):
    ...
    for i in indices:
        blocks[i] = _add_polarization_orb(blocks[i])
...
def generate_basis(atoms, xcf='PBEsol', basis='DZPp',
    ...
    if add_p:
        targets = [symbols[0]]
        block = modify_basis(block, targets)
