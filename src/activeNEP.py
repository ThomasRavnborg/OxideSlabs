...
class ActiveLearningNEP:
    ...
    def setup_nep(self, cutoff=[8, 4], neuron=30, generation=100000, batch=1000000):
        ...
        if self.nep_txt is not None:
            print("NEP model already exists.")
            return

        nep_dir = os.path.join(self.iter_dir, "nep")
        try:
            os.makedirs(nep_dir, exist_ok=False)
        except Exception:
    ...
    def train_nep(self):
        if self.nep_txt is not None:
            print("Existing NEP model found. NEP training skipped.")
            return
        nep_dir = os.path.join(self.iter_dir, "nep")
        nep_txt = os.path.join(self.iter_dir, "nep.txt")
    ...
    def assign_descriptors(self, structures):
        ...
        if self.nep_txt is None:
            raise RuntimeError("NEP model not found. Train NEP first.")
