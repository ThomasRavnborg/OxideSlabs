...
def plot_bands(formula, ids=[], mode='lcao'):
    ...
    for i, id in enumerate(ids):
        dir = os.path.join('results/bulk/',formula, id, 'bands')
        ...
        if i == 0:
            ...
            ax1.xaxis.set_ticks(X[0])
            ax1.set_xticklabels(X[1])
            # Plot vertical lines at symmetry points
            ax1.vlines(X[0], ytickmarks[0], ytickmarks[-1], color='0.5', linewidth=1)
        ...
        for j, e_k in enumerate(bands):
            if j == 0:
                ax1.plot(x, e_k, color=col, label=f"ID {id}")
            else:
                ax1.plot(x, e_k, color=col)
        ...
        ax2.plot(DOS, E, color=col, label=f"ID {id}")
