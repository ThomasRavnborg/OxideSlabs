...
def calculate_bands(atoms, xcf='PBEsol', basis='DZP', EnergyShift=0.01, SplitNorm=0.15,
                    ...
                    mode='lcao', dir='results/bulk/bandstructure', par=False):
    ...
    if mode == 'pw':
        # Calculate bandstructure along the path a path (in this case 'GXRMGR')    
...
def plot_bands(formula, ids=np.array([]), vals=np.array([]), root='results'):
    ...
    - root: Root directory where the data is stored (default is 'results').
    Returns:
    ...
    def _plot_bandstructure(ax, dir, val, col='k', mode='lcao'):
        ...
        x /= x[-1]     # Normalize x to the total length of the path
    ...
    def _plot_dos(ax, dir, val, col='k', mode='lcao'):
        if mode == 'lcao':
        ...
        ax.plot(DOS, E, color=col, label=f"{val}")
